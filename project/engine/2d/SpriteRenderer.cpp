...
SpriteRenderer* SpriteRenderer::GetInstance() {
	if (instance == nullptr) {
		instance = std::make_unique<SpriteRenderer>();
	}
...
/// 共通描画設定
/// </summary>
void SpriteRenderer::SetupCommonRenderState(BlendMode blendMode) {
	// ルートシグネチャをセット
...
void SpriteRenderer::CreateRootSignature() {
	...
	staticSamplers[0].ComparisonFunc =
		D3D12_COMPARISON_FUNC_NEVER;              //   比較しない
...
void SpriteRenderer::CreateGraphicsPipeline() {
	...
	for (int i = 0; i < static_cast<int>(BlendMode::kCountOfBlendMode); ++i) {
		...
		HRESULT hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(
			&graphicsPipelineStateDesc, IID_PPV_ARGS(&graphicsPipelineStates_[i]));
		assert(SUCCEEDED(hr));
	}
}
...
/// </summary>
D3D12_BLEND_DESC SpriteRenderer::MakeBlendDesc(BlendMode mode) {
