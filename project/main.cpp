...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  //
  //  HWND hwnd =
  //      CreateWindow(wc.lpszClassName, L"CG2", WS_OVERLAPPEDWINDOW,
  //      CW_USEDEFAULT,
  //                   CW_USEDEFAULT, wrc.right - wrc.left, wrc.bottom -
  //                   wrc.top, nullptr, nullptr, wc.hInstance, nullptr);
  //
  // #ifdef _DEBUG
  ...
  rtvHandles[1] =
      GetCPUDescriptorHandle(rtvDescriptorHeap, descriptorSizeRTV, 1);
  ...
  for (int latIndex = 0; latIndex < kSubdivision; ++latIndex) {
    // このセルの上下緯度と V
    float lat0 = -static_cast<float>(M_PI) / 2.0f + kLatEvery * latIndex; // 上
    float lat1 = lat0 + kLatEvery;                                        // 下
    ...
    for (int lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
      ...
      vertexData[start] = {{std::cos(lat0) * std::cos(lon0), std::sin(lat0),
                           ...
                           {u0, v0}};
      vertexData[start + 1] = {{std::cos(lat1) * std::cos(lon0), std::sin(lat1),
                                std::cos(lat1) * std::sin(lon0), 1.0f},
                               {u0, v1}};
      vertexData[start + 2] = {{std::cos(lat0) * std::cos(lon1), std::sin(lat0),
                                std::cos(lat0) * std::sin(lon1), 1.0f},
                               {u1, v0}};

      // ── 三角形２ （右上→左下→右下）
      vertexData[start + 3] = {{std::cos(lat0) * std::cos(lon1), std::sin(lat0),
                                std::cos(lat0) * std::sin(lon1), 1.0f},
                               {u1, v0}};
      vertexData[start + 4] = {{std::cos(lat1) * std::cos(lon0), std::sin(lat1),
                                std::cos(lat1) * std::sin(lon0), 1.0f},
                               {u0, v1}};
      vertexData[start + 5] = {{std::cos(lat1) * std::cos(lon1), std::sin(lat1),
  ...
  transformMatrixResource->Map(0, nullptr,
                               reinterpret_cast<void **>(&transformMatrixData));
  // 単位行列を書き込んでおく
  ...
  intermediateResources[0] =
      UploadTextureData(textureResources[0], mipImages[0], device, commandList);
  ...
  intermediateResources[1] =
      UploadTextureData(textureResources[1], mipImages[1], device, commandList);
  ...
  for (int i = 0; i < kNumTextures; ++i) {
    ...
    srvDesc.Texture2D.MipLevels = static_cast<UINT>(metadata[i].mipLevels);
  ...
  while (true) {
    ...
    // UIの移動
    ImGui::Text("UI");
    ImGui::SliderFloat3("translationSprite", &transformSprite.translate.x, 0.0f,
                        1280.0f);
